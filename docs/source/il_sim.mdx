...
4. We set `mlflow.enable=true` to stream metrics to [MLflow](https://mlflow.org/docs/latest/index.html). This is optional; leave it disabled to keep metrics local, or set `mlflow.tracking_uri` to send runs to your MLflow server.
