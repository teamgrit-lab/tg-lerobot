...
- A successful experiment is one where the human has to intervene at the start but then reduces the amount of interventions as the policy improves. You can monitor the intervention rate in the MLflow dashboard when tracking is enabled.
...
If you have `mlflow.enable` set to `true` in your configuration, you can monitor training progress in real time through your [MLflow](https://mlflow.org/docs/latest/index.html) instance.
