...
4. We set `mlflow.enable=true` to stream training metrics to [MLflow](https://mlflow.org/docs/latest/index.html). This is optional—if disabled, logs stay local. Configure `mlflow.tracking_uri` to send runs to an existing MLflow server.
